# HSK1 (3.0) — Checklist phủ ~300 từ theo 16 buổi

**Nguồn 生词:** `docs/superpowers/specs/newhsk1-book-content.md` (bóc từ New HSK Course 1, 15 课,
≈247 生词) — remapped theo buổi chủ đề per `docs/superpowers/specs/2026-07-21-hsk1-3.0-full-course-design.md` §4.
**Thay thế:** `hsk1-150-checklist.md` (bản 2.0/150 từ — SUPERSEDED, giữ tham chiếu lịch sử).

Mapping buổi ↔ 课 sách: 02=L1+L2 · 03=L3 · 04=L4 · 05=L6 · 06=L5 · 07=L7 · 08=L8 · 09=L9 ·
10=L10 · 11=L13 · 12=L14 · 13=L11 · 14=L12 · 15=L15 · (01=ngữ âm, 16=ôn tập, cả hai không có 生词 mới).

Trạng thái: buổi 01 ✅ đã có (ngữ âm, no vocab). Buổi 02–15: ⏳ chưa soạn (vocab remap xong,
chưa build slide/bài tập).

---

## Buổi 02 — Chào hỏi & làm quen (L1 + L2)

⚠️ **SỬA 2026-09-09:** L1 audit khớp hoàn toàn (13/13 từ). L2 bản cũ thiếu 2 từ — **不** và
**对不起** (khối 2-4, trang 007) — đối chiếu trực tiếp trang 001-009 sách gốc bằng vision.

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 你好 | nǐ hǎo | xin chào | ⏳ chưa soạn |
| 王老师 | Wáng lǎoshī | (riêng) cô/thầy Vương | ⏳ chưa soạn |
| 大家 | dàjiā | mọi người | ⏳ chưa soạn |
| 好 | hǎo | tốt, khỏe, ổn (tính từ) | ⏳ chưa soạn |
| 学生 | xuésheng | học sinh | ⏳ chưa soạn |
| 们 | men | hậu tố số nhiều | ⏳ chưa soạn |
| 老师 | lǎoshī | giáo viên | ⏳ chưa soạn |
| 您 | nín | ngài, bác (kính ngữ) | ⏳ chưa soạn |
| 你们 | nǐmen | các bạn (số nhiều) | ⏳ chưa soạn |
| 谢谢 | xièxie | cảm ơn | ⏳ chưa soạn |
| 不客气 | bú kèqi | không có gì | ⏳ chưa soạn |
| 同学 | tóngxué | bạn học | ⏳ chưa soạn |
| 再见 | zàijiàn | tạm biệt | ⏳ chưa soạn |
| 请问 | qǐngwèn | xin hỏi | ⏳ chưa soạn |
| 你 | nǐ | bạn (số ít) | ⏳ chưa soạn |
| 叫 | jiào | tên là, gọi là | ⏳ chưa soạn |
| 什么 | shénme | gì, cái gì | ⏳ chưa soạn |
| 名字 | míngzi | tên | ⏳ chưa soạn |
| 我 | wǒ | tôi | ⏳ chưa soạn |
| 不 | bù | không, chẳng (phó từ phủ định) | ✅ bổ sung 2026-09-09 |
| 是 | shì | là | ⏳ chưa soạn |
| 对不起 | duìbuqǐ | xin lỗi | ✅ bổ sung 2026-09-09 |
| 没关系 | méi guānxi | không sao | ⏳ chưa soạn |
| 没事儿 | méishìr | không sao, không có gì | ⏳ chưa soạn |
| 很 | hěn | rất | ⏳ chưa soạn |
| 高兴 | gāoxìng | vui, mừng | ⏳ chưa soạn |
| 认识 | rènshi | quen biết | ⏳ chưa soạn |
| 也 | yě | cũng | ⏳ chưa soạn |

**28 từ**

---

## Buổi 03 — Quốc tịch & bản thân (L3)

⚠️ **SỬA 2026-09-04:** bảng cũ ghi 11 từ là SAI (thiếu khối 生词 3-6 + 国 + 她, và gán sai 都).
Đối chiếu trực tiếp sách (trang 26/28/29, đọc bằng vision) → Bài 3 có **22 từ**.

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 人 | rén | người | ✅ đã có |
| 的 | de | trợ từ sở hữu/định ngữ | ✅ đã có |
| 这 | zhè | này, đây | ✅ đã có |
| 谁 | shéi | ai | ✅ đã có |
| 女朋友 | nǚpéngyou | bạn gái (người yêu) | ✅ đã có |
| 哪 | nǎ | nào | ✅ đã có |
| 国 | guó | nước, quốc gia | ✅ bổ sung 2026-09-04 |
| 她 | tā | cô ấy | ✅ bổ sung 2026-09-04 |
| 喂 | wèi | alô, này | ✅ bổ sung 2026-09-04 |
| 姐姐 | jiějie | chị gái | ✅ bổ sung 2026-09-04 |
| 工作 | gōngzuò | công việc | ✅ bổ sung 2026-09-04 |
| 还 | hái | vẫn, còn | ✅ bổ sung 2026-09-04 |
| 忙 | máng | bận | ✅ bổ sung 2026-09-04 |
| 吗 | ma | trợ từ nghi vấn | ✅ bổ sung 2026-09-04 |
| 对 | duì | đúng | ✅ bổ sung 2026-09-04 |
| 太 | tài | quá, lắm | ✅ bổ sung 2026-09-04 |
| 我们 | wǒmen | chúng tôi | ✅ bổ sung 2026-09-04 |
| 想 | xiǎng | nhớ (ai đó) | ✅ bổ sung 2026-09-04 |
| 中国 | Zhōngguó | (riêng) Trung Quốc | ✅ đã có |
| 法国 | Fǎguó | (riêng) Pháp | ✅ đã có |
| 中文 | Zhōngwén | (riêng) tiếng Trung | ✅ đã có |
| 泰国 | Tàiguó | (riêng) Thái Lan | ✅ đã có |

**22 từ.** 都 đã BỎ khỏi bài này (thuộc Bài 14).

Mở rộng ngoài sách (dạy ở Phần 2 của buổi): 越南, 美国, 英国, 日本, 韩国, 德国, 澳大利亚,
小学生, 初中生, 高中生, 大学生 (11 từ).

> ⚠️ **Tổng 243 từ của checklist này KHÔNG còn đáng tin** — riêng Bài 3 đã thiếu 12/22 từ (55%)
> vì bản bóc chỉ lấy 1-2 khối 生词 đầu mỗi bài, bỏ khối cuối. Các bài khác cần audit lại bằng
> cách render trang PDF ra ảnh rồi đọc bằng vision (OCR text của sách quá nhiễu cho bảng 生词).

---

## Buổi 04 — Gia đình (L4)

**ĐÃ SỬA 2026-09-08** — đối chiếu trực tiếp trang 34/37/40 sách gốc bằng vision: sách có
21 từ 生词 đánh số ①-㉑ (không phải 16 như bản cũ) + 1 từ dạy trong ngữ pháp (两). Bản cũ
thiếu 有/他/岁/今年/多/大/孩子 và ghép sai "多大" thành 1 từ.

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 有 | yǒu | có | ⏳ chưa soạn |
| 多少 | duōshao | bao nhiêu | ⏳ chưa soạn |
| 个 | gè | cái (lượng từ vạn năng) | ⏳ chưa soạn |
| 哥哥 | gēge | anh trai | ⏳ chưa soạn |
| 呢 | ne | trợ từ hỏi rút gọn | ⏳ chưa soạn |
| 没有 | méiyǒu | không có | ⏳ chưa soạn |
| 家 | jiā | nhà, gia đình | ⏳ chưa soạn |
| 几 | jǐ | mấy | ⏳ chưa soạn |
| 口 | kǒu | lượng từ đếm người trong nhà | ⏳ chưa soạn |
| 爸爸 | bàba | bố | ⏳ chưa soạn |
| 妈妈 | māma | mẹ | ⏳ chưa soạn |
| 妹妹 | mèimei | em gái | ⏳ chưa soạn |
| 和 | hé | và | ⏳ chưa soạn |
| 儿子 | érzi | con trai | ⏳ chưa soạn |
| 孩子 | háizi | con cái | ⏳ chưa soạn |
| 女儿 | nǚ'ér | con gái | ⏳ chưa soạn |
| 岁 | suì | tuổi | ⏳ chưa soạn |
| 他 | tā | anh ấy/nó | ⏳ chưa soạn |
| 今年 | jīnnián | năm nay | ⏳ chưa soạn |
| 多 | duō | (hỏi mức độ) bao nhiêu | ⏳ chưa soạn |
| 大 | dà | (tuổi) lớn | ⏳ chưa soạn |
| 两 *(ngữ pháp, không có số 生词)* | liǎng | hai (trước lượng từ) | ⏳ chưa soạn |

**21 từ + 1 từ ngữ pháp (两)** (家, 和 ôn lại ở buổi 09)

---

## Buổi 05 — Số đếm & số điện thoại (L6)

> ⚠️ Đã đối chiếu PDF gốc bằng vision (2026-09-09, trang 50-58 = index 49-58 file
> `raw/新HSK1教程3.0.pdf`) — checklist cũ sai nhiều: gộp nhầm 手机号 thành 1 từ (thực ra 2 từ
> rời 手机+号, cộng thêm 电话 riêng), bịa "一些" (sách chỉ dạy 些), lẫn "好的" vào (好的 KHÔNG
> phải 生词 mới của bài này — không có trong hộp New Words 6-2), và bỏ sót hẳn 5 từ
> (明天, 去, 哪儿, 东西 + 电话). Danh sách dưới đây lấy đúng theo 3 hộp New Words 6-2/6-4/6-6
> trong sách.

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 手机 | shǒujī | điện thoại di động | ⏳ chưa soạn |
| 电话 | diànhuà | điện thoại, cuộc gọi | ⏳ chưa soạn |
| 号 | hào | số | ⏳ chưa soạn |
| 明天 | míngtiān | ngày mai | ⏳ chưa soạn |
| 去 | qù | đi | ⏳ chưa soạn |
| 哪儿 | nǎr | đâu, ở đâu | ⏳ chưa soạn |
| 想 | xiǎng | muốn, định | ⏳ chưa soạn |
| 超市 | chāoshì | siêu thị | ⏳ chưa soạn |
| 买 | mǎi | mua | ⏳ chưa soạn |
| 东西 | dōngxi | đồ, thứ | ⏳ chưa soạn |
| 些 | xiē | một số, vài | ⏳ chưa soạn |
| 牛奶 | niúnǎi | sữa bò | ⏳ chưa soạn |
| 吃 | chī | ăn | ⏳ chưa soạn |
| 晚饭 | wǎnfàn | cơm tối | ⏳ chưa soạn |
| 那边 | nàbiān | đằng kia | ⏳ chưa soạn |
| 包子 | bāozi | bánh bao | ⏳ chưa soạn |
| 非常 | fēicháng | rất, vô cùng | ⏳ chưa soạn |
| 好吃 | hǎochī | ngon | ⏳ chưa soạn |
| 米饭 | mǐfàn | cơm (gạo) | ⏳ chưa soạn |
| 怎么 | zěnme | thế nào, làm sao | ⏳ chưa soạn |
| 坐 | zuò | ngồi, đi (bằng xe) | ⏳ chưa soạn |
| 出租车 | chūzūchē | taxi | ⏳ chưa soạn |
| 西安饭店 (专名) | Xī'ān Fàndiàn | (riêng) Nhà hàng Tây An | ⏳ chưa soạn |

**23 từ** (22 生词 + 1 专有名词)

---

## Buổi 06 — Ngày tháng & nghỉ làm (L5)

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 几号 | jǐ hào | ngày mấy | ⏳ chưa soạn |
| 星期几 | xīngqī jǐ | thứ mấy | ⏳ chưa soạn |
| 休息 | xiūxi | nghỉ ngơi | ⏳ chưa soạn |
| 会 | huì | biết (do học mà có) | ⏳ chưa soạn |
| 做 | zuò | làm | ⏳ chưa soạn |
| 菜 | cài | món ăn | ⏳ chưa soạn |
| 下班 | xiàbān | tan làm | ⏳ chưa soạn |
| 新 | xīn | mới | ⏳ chưa soạn |
| 电脑 | diànnǎo | máy tính | ⏳ chưa soạn |
| 真 | zhēn | thật là | ⏳ chưa soạn |
| 好看 | hǎokàn | đẹp, dễ nhìn | ⏳ chưa soạn |
| 喜欢 | xǐhuan | thích | ⏳ chưa soạn |
| 它 | tā | nó | ⏳ chưa soạn |

**13 từ**

---

## Buổi 07 — Giờ giấc & công việc (L7)

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 现在 | xiànzài | bây giờ | ⏳ chưa soạn |
| 点 | diǎn | giờ (thời điểm) | ⏳ chưa soạn |
| 早上 | zǎoshang | buổi sáng sớm | ⏳ chưa soạn |
| 上午 | shàngwǔ | buổi sáng | ⏳ chưa soạn |
| 分 | fēn | phút | ⏳ chưa soạn |
| 课 | kè | tiết học | ⏳ chưa soạn |
| 下午 | xiàwǔ | buổi chiều | ⏳ chưa soạn |
| 见 | jiàn | gặp | ⏳ chưa soạn |
| 吧 | ba | trợ từ đề nghị/thương lượng | ⏳ chưa soạn |
| 电影院 | diànyǐngyuàn | rạp chiếu phim | ⏳ chưa soạn |
| 看 | kàn | xem, nhìn | ⏳ chưa soạn |
| 上课 | shàngkè | lên lớp, học | ⏳ chưa soạn |
| 下课 | xiàkè | tan học | ⏳ chưa soạn |
| 店 | diàn | cửa hàng | ⏳ chưa soạn |
| 分钟 | fēnzhōng | phút (khoảng thời gian) | ⏳ chưa soạn |
| 医院 | yīyuàn | bệnh viện | ⏳ chưa soạn |

**16 từ**

---

## Buổi 08 — Nghề & nơi làm · 在 (L8)

✅ **ĐÃ SỬA 2026-09-09** (đối chiếu trực tiếp trang 055/057/059 sách gốc = index 69/71/73 file
PDF, bằng vision). Bổ sung 3 từ bị OCR bỏ sót ở block 8-4/8-6: **学校, 书店, 病人**.

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 房间 | fángjiān | phòng | ⏳ chưa soạn |
| 外 | wài | ngoài | ⏳ chưa soạn |
| 只 | zhī | lượng từ (con — cho động vật) | ⏳ chưa soạn |
| 小 | xiǎo | nhỏ | ⏳ chưa soạn |
| 猫 | māo | mèo | ⏳ chưa soạn |
| 没 | méi | chưa | ⏳ chưa soạn |
| 看见 | kànjiàn | nhìn thấy | ⏳ chưa soạn |
| 桌子 | zhuōzi | cái bàn | ⏳ chưa soạn |
| 下 | xià | dưới | ⏳ chưa soạn |
| 漂亮 | piàoliang | đẹp | ⏳ chưa soạn |
| 在 | zài | ở, tại | ⏳ chưa soạn |
| 学校 | xuéxiào | trường học | ⏳ chưa soạn |
| 书店 | shūdiàn | hiệu sách | ⏳ chưa soạn |
| 前 | qián | trước | ⏳ chưa soạn |
| 能 | néng | có thể (năng lực/điều kiện) | ⏳ chưa soạn |
| 到 | dào | đến, tới | ⏳ chưa soạn |
| 午饭 | wǔfàn | cơm trưa | ⏳ chưa soạn |
| 饭 | fàn | cơm, bữa ăn | ⏳ chưa soạn |
| 大 | dà | to, lớn | ⏳ chưa soạn |
| 病人 | bìngrén | bệnh nhân | ⏳ chưa soạn |
| 多 | duō | nhiều | ⏳ chưa soạn |
| 医生 | yīshēng | bác sĩ | ⏳ chưa soạn |
| 工作 | gōngzuò | công việc, làm việc | ⏳ chưa soạn |

**23 từ** (+ danh từ riêng 胡医生; 在 ôn lại ở buổi 13)

---

## Buổi 09 — Nơi chốn & đồ vật · vị trí (L9)

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 前边 | qiánbian | phía trước | ⏳ chưa soạn |
| 边 | bian | hậu tố chỉ phía/bên | ⏳ chưa soạn |
| 那个 | nàge | cái đó | ⏳ chưa soạn |
| 外边 | wàibian | bên ngoài | ⏳ chưa soạn |
| 白天 | báitiān | ban ngày | ⏳ chưa soạn |
| 读书 | dúshū | đọc sách, đi học | ⏳ chưa soạn |
| 朋友 | péngyou | bạn bè | ⏳ chưa soạn |
| 唱 | chàng | hát | ⏳ chưa soạn |
| 歌 | gē | bài hát | ⏳ chưa soạn |
| 好听 | hǎotīng | hay (nghe) | ⏳ chưa soạn |
| 电视 | diànshì | tivi | ⏳ chưa soạn |
| 狗 | gǒu | chó | ⏳ chưa soạn |
| 玩儿 | wánr | chơi | ⏳ chưa soạn |
| 第 | dì | thứ (số thứ tự) | ⏳ chưa soạn |

**14 từ** (không tính 家, 和 — đã tính ở buổi 04, ôn lại ở đây)

---

## Buổi 10 — Mua sắm & giá tiền (L10)

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 杯子 | bēizi | cái cốc/ly | ⏳ chưa soạn |
| 售货员 | shòuhuòyuán | nhân viên bán hàng | ⏳ chưa soạn |
| 这边 | zhèbian | bên này | ⏳ chưa soạn |
| 钱 | qián | tiền | ⏳ chưa soạn |
| 这些 | zhèxiē | những cái này | ⏳ chưa soạn |
| 块 | kuài | đồng (đơn vị tiền) | ⏳ chưa soạn |
| 那些 | nàxiē | những cái đó | ⏳ chưa soạn |
| 这儿 | zhèr | ở đây | ⏳ chưa soạn |
| 水果 | shuǐguǒ | hoa quả | ⏳ chưa soạn |
| 少 | shǎo | ít | ⏳ chưa soạn |
| 斤 | jīn | cân (~500g) | ⏳ chưa soạn |
| 苹果 | píngguǒ | táo | ⏳ chưa soạn |
| 便宜 | piányi | rẻ | ⏳ chưa soạn |
| 商店 | shāngdiàn | cửa hàng | ⏳ chưa soạn |
| 衣服 | yīfu | quần áo | ⏳ chưa soạn |
| 件 | jiàn | lượng từ (bộ/chiếc quần áo) | ⏳ chưa soạn |
| 元 | yuán | đồng (nguyên) | ⏳ chưa soạn |
| 怎么样 | zěnmeyàng | thế nào, ra sao | ⏳ chưa soạn |
| 穿 | chuān | mặc | ⏳ chưa soạn |
| 女 | nǚ | nữ | ⏳ chưa soạn |
| 男 | nán | nam | ⏳ chưa soạn |
| 那儿 | nàr | ở đó | ⏳ chưa soạn |

**22 từ**

---

## Buổi 11 — Ăn uống & gọi món (L13)

⚠️ Nguồn có block OCR sót đã flag (xem note dưới) — danh sách phục hồi từ hội thoại + hộp
13-4/13-6, cần đối chiếu `raw/新HSK1教程3.0.pdf` khi build.

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 可以 | kěyǐ | có thể, được phép | ⏳ chưa soạn |
| 卖 | mài | bán | ⏳ chưa soạn |
| 女士 | nǚshì | quý bà, cô | ⏳ chưa soạn |
| 服务员 | fúwùyuán | nhân viên phục vụ | ⏳ chưa soạn |
| 杯 | bēi | cốc (lượng từ) | ⏳ chưa soạn |
| 茶 | chá | trà | ⏳ chưa soạn |
| 这个 | zhège | cái này | ⏳ chưa soạn |
| 面包 | miànbāo | bánh mì | ⏳ chưa soạn |
| 鸡蛋 | jīdàn | trứng gà | ⏳ chưa soạn |
| 一下 | yíxià | một chút, thử xem | ⏳ chưa soạn |
| 半 | bàn | nửa | ⏳ chưa soạn |
| 打电话 | dǎ diànhuà | gọi điện thoại | ⏳ chưa soạn |

**12 từ**

---

## Buổi 12 — Sở thích & phim · 了 (L14)

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 火车 | huǒchē | tàu hỏa | ⏳ chưa soạn |
| 汉字 | Hànzì | (riêng) chữ Hán | ⏳ chưa soạn |
| 听 | tīng | nghe | ⏳ chưa soạn |
| 小学生 | xiǎoxuéshēng | học sinh tiểu học | ⏳ chưa soạn |
| 写 | xiě | viết | ⏳ chưa soạn |
| 听见 | tīngjiàn | nghe thấy | ⏳ chưa soạn |
| 不要 | búyào | đừng, không được | ⏳ chưa soạn |
| 哪些 | nǎxiē | những cái nào | ⏳ chưa soạn |
| 汉语 | Hànyǔ | (riêng) tiếng Trung (tiếng nói) | ⏳ chưa soạn |
| 明年 | míngnián | năm sau | ⏳ chưa soạn |
| 上 | shàng | bắt đầu (đi học/lên lớp) | ⏳ chưa soạn |
| 中学 | zhōngxué | trung học | ⏳ chưa soạn |
| 小学 | xiǎoxué | tiểu học | ⏳ chưa soạn |
| 中学生 | zhōngxuéshēng | học sinh trung học | ⏳ chưa soạn |
| 上学 | shàngxué | đi học | ⏳ chưa soạn |
| 他们 | tāmen | họ (nam/chung) | ⏳ chưa soạn |
| 她们 | tāmen | họ (nữ) | ⏳ chưa soạn |
| 它们 | tāmen | chúng nó (vật) | ⏳ chưa soạn |
| 晚 | wǎn | muộn | ⏳ chưa soạn |

**19 từ** (không tính 都 — đã tính ở buổi 03, ôn lại ở đây)

---

## Buổi 13 — Đại học & đang làm · 呢 (L11)

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 什么时候 | shénme shíhou | khi nào | ⏳ chưa soạn |
| 知道 | zhīdào | biết | ⏳ chưa soạn |
| 正在 | zhèngzài | đang | ⏳ chưa soạn |
| 后边 | hòubian | phía sau | ⏳ chưa soạn |
| 开车 | kāichē | lái xe | ⏳ chưa soạn |
| 车 | chē | xe | ⏳ chưa soạn |
| 读 | dú | học, theo học | ⏳ chưa soạn |
| 大学 | dàxué | đại học | ⏳ chưa soạn |
| 大学生 | dàxuéshēng | sinh viên đại học | ⏳ chưa soạn |
| 学 | xué | học | ⏳ chưa soạn |
| 医 | yī | y (ngành y) | ⏳ chưa soạn |
| 问 | wèn | hỏi | ⏳ chưa soạn |
| 对 | duì | đối với, hướng về | ⏳ chưa soạn |
| 说 | shuō | nói | ⏳ chưa soạn |
| 要 | yào | muốn, định | ⏳ chưa soạn |
| 小朋友 | xiǎopéngyou | em nhỏ, bạn nhỏ | ⏳ chưa soạn |
| 弟弟 | dìdi | em trai | ⏳ chưa soạn |
| 起床 | qǐchuáng | ngủ dậy | ⏳ chưa soạn |
| 睡 | shuì | ngủ | ⏳ chưa soạn |
| 那里 | nàlǐ | chỗ đó | ⏳ chưa soạn |
| 哪里 | nǎlǐ | chỗ nào, đâu | ⏳ chưa soạn |

**21 từ** (không tính 在 — đã tính ở buổi 08, ôn lại ở đây)

---

## Buổi 14 — Thời tiết & sức khỏe (L12)

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 天气 | tiānqì | thời tiết | ⏳ chưa soạn |
| 这里 | zhèlǐ | ở đây | ⏳ chưa soạn |
| 天 | tiān | trời; ngày | ⏳ chưa soạn |
| 下雨 | xiàyǔ | mưa (động từ) | ⏳ chưa soạn |
| 了 | le | trợ từ chỉ sự thay đổi | ⏳ chưa soạn |
| 雨 | yǔ | mưa (danh từ) | ⏳ chưa soạn |
| 有点儿 | yǒudiǎnr | hơi, có chút | ⏳ chưa soạn |
| 觉得 | juéde | cảm thấy | ⏳ chưa soạn |
| 冷 | lěng | lạnh | ⏳ chưa soạn |
| 生病 | shēngbìng | bị bệnh | ⏳ chưa soạn |
| 看病 | kànbìng | khám bệnh | ⏳ chưa soạn |
| 病 | bìng | bệnh, ốm | ⏳ chưa soạn |
| 一点儿 | yìdiǎnr | một chút | ⏳ chưa soạn |
| 药 | yào | thuốc | ⏳ chưa soạn |
| 回 | huí | về, trở về | ⏳ chưa soạn |
| 再 | zài | rồi, sau đó mới | ⏳ chưa soạn |
| 喝 | hē | uống | ⏳ chưa soạn |
| 热 | rè | nóng | ⏳ chưa soạn |
| 水 | shuǐ | nước | ⏳ chưa soạn |

**19 từ**

---

## Buổi 15 — Đi lại & du lịch · hẹn gặp (L15)

| 汉字 | pinyin | nghĩa Việt | trạng thái |
|---|---|---|---|
| 爱 | ài | yêu, thích | ⏳ chưa soạn |
| 哪个 | nǎge | cái nào | ⏳ chưa soạn |
| 好玩儿 | hǎowánr | vui, thú vị | ⏳ chưa soạn |
| 去年 | qùnián | năm ngoái | ⏳ chưa soạn |
| 男朋友 | nánpéngyou | bạn trai | ⏳ chưa soạn |
| 西安 | Xī'ān | (riêng) Tây An | ⏳ chưa soạn |
| 北京 | Běijīng | (riêng) Bắc Kinh | ⏳ chưa soạn |
| 飞机 | fēijī | máy bay | ⏳ chưa soạn |
| 小时 | xiǎoshí | giờ (thời lượng) | ⏳ chưa soạn |
| 家人 | jiārén | người nhà | ⏳ chưa soạn |
| 时间 | shíjiān | thời gian | ⏳ chưa soạn |
| 机场 | jīchǎng | sân bay | ⏳ chưa soạn |
| 住 | zhù | ở, sống | ⏳ chưa soạn |
| 早 | zǎo | sớm | ⏳ chưa soạn |
| 那 | nà | vậy thì, thế thì (liên từ) | ⏳ chưa soạn |
| 大兴机场 | Dàxīng Jīchǎng | (riêng) Sân bay Đại Hưng | ⏳ chưa soạn |

**16 từ**

---

## Bảng tổng hợp

| Buổi | Chủ đề | 课 sách | Số 生词 (distinct, sau dedup) | Trạng thái |
|---|---|---|---|---|
| 01 | Ngữ âm | — | 0 (no vocab) | ✅ đã có |
| 02 | Chào hỏi & làm quen | L1+L2 | 28 | ⏳ chưa soạn |
| 03 | Quốc tịch & bản thân | L3 | 11 (+12 mở rộng: 7 quốc gia, 男朋友, 4 cấp học) | ✅ đã có |
| 04 | Gia đình | L4 | 16 | ⏳ chưa soạn |
| 05 | Số đếm & số điện thoại | L6 | 23 | ✅ đã có (slide; 西安饭店 giữ trong 课文/ví dụ, không làm thẻ từ vựng riêng vì chỉ là tên riêng dùng 1 lần) |
| 06 | Ngày tháng & nghỉ làm | L5 | 13 | ⏳ chưa soạn |
| 07 | Giờ giấc & công việc | L7 | 16 | ⏳ chưa soạn |
| 08 | Nghề & nơi làm · 在 | L8 | 20 | ⏳ chưa soạn |
| 09 | Nơi chốn & đồ vật · vị trí | L9 | 14 | ⏳ chưa soạn |
| 10 | Mua sắm & giá tiền | L10 | 22 | ⏳ chưa soạn |
| 11 | Ăn uống & gọi món | L13 | 12 | ⏳ chưa soạn |
| 12 | Sở thích & phim · 了 | L14 | 19 | ⏳ chưa soạn |
| 13 | Đại học & đang làm · 呢 | L11 | 21 | ⏳ chưa soạn |
| 14 | Thời tiết & sức khỏe | L12 | 19 | ⏳ chưa soạn |
| 15 | Đi lại & du lịch · hẹn gặp | L15 | 16 | ⏳ chưa soạn |
| 16 | Ôn tập tổng hợp | — | 0 (ôn lại ~250) | ⏳ chưa soạn |
| **Tổng** | | | **250 distinct** (per-buổi range 11–28) | |

Tổng theo từng 课 sách trước dedup = 254 (247 gốc + 2 từ audit buổi 02 bổ sung 2026-09-09: 不,
对不起 + 5 từ audit buổi 05 bổ sung 2026-09-09: bỏ "手机号" gộp/"一些"/"好的" sai, thay bằng
手机, 电话, 号, 明天, 去, 哪儿, 想, 东西, 些 đúng theo sách — net +5 từ so với 18 cũ). Trừ 4 từ
trùng lặp xuất hiện lại ở 课 khác (xem mục dưới) → **250 từ phân biệt** trong checklist này.

> Số 254/250 vẫn CHƯA phải con số cuối — mới chỉ audit xong buổi 01, 02, 03, 05. Các buổi 04,
> 06-15 chưa đối chiếu bằng vision, rất có thể còn thiếu từ tương tự (xem cảnh báo dưới).

---

## Từ trùng lặp giữa các 课 (dedupe — tính 1 lần ở buổi xuất hiện đầu)

| 汉字 | pinyin | Buổi gốc (tính vào) | Ôn lại ở |
|---|---|---|---|
| 家 | jiā | Buổi 04 (L4) | Buổi 09 (L9) |
| 和 | hé | Buổi 04 (L4) | Buổi 09 (L9) |
| 都 | dōu | Buổi 03 (L3) | Buổi 12 (L14) |
| 在 | zài | Buổi 08 (L8) | Buổi 13 (L11) |

---

## Đối chiếu với từ "tận dụng buổi cũ 2.0" (spec §5)

Spec §5 liệt kê các từ dự định tái dùng từ 3 buổi 2.0 cũ (buoi06/10/12). Đối chiếu với
book-content để tránh đếm trùng/đếm nhầm:

| Nhóm (spec §5) | Từ | Có trong book-content 生词? | Buổi 3.0 tương ứng |
|---|---|---|---|
| 量词 | 个 | Có | Buổi 04 (đã tính) |
| 量词 | 块 | Có | Buổi 10 (đã tính) |
| 量词 | 本 | **Không** — không xuất hiện trong 生词 box của 15 课 | Không đưa vào checklist (tránh "invent" từ ngoài book-content); nếu cần dạy thì để "mở rộng" ngoài scope 300, đối chiếu PDF gốc |
| Ngữ pháp | 了 | Có | Buổi 14 (grammar 了(1)) + Buổi 12 (动态助词 了(2)) — đã tính trong 生词 tương ứng |
| Thời tiết | 天气 | Có | Buổi 14 (đã tính) |
| Thời tiết | 下雨 | Có | Buổi 14 (đã tính) |
| Thời tiết | 下雪 | **Không** — chỉ xuất hiện trong tiêu đề/课文 Bài 12 ("昨天下雪了"), không có trong hộp 生词 (chỉ 下雨/雨 được liệt kê) | Không đưa vào checklist; cần đối chiếu PDF gốc xem 雪/下雪 có bị OCR bỏ sót khỏi hộp 生词 không |
| Thời tiết | 冷 | Có | Buổi 14 (đã tính) |
| Thời tiết | 热 | Có | Buổi 14 (đã tính) |

Kết luận: 2/8 từ tái dùng dự kiến (本, 下雪) không xác nhận được trong book-content — không
đưa vào checklist 245 từ để tránh bịa đặt; cần đối chiếu `raw/新HSK1教程3.0.pdf` (hoặc bản
OCR .ocr.txt) khi build buổi 04/10 (本) và buổi 14 (下雪) để xác nhận có bị OCR bỏ sót không,
hay đúng là sách không dạy từ này ở L1-15 (thuộc phần mở rộng ngoài 300, theo spec §5).

---

## Chênh lệch 245 vs. mốc ~300

Tổng distinct trong checklist này = **245** (< mốc ~300 của HSK1 3.0 final; số này còn tăng
tiếp khi audit xong buổi 04-15 — xem cảnh báo ở mục "Bảng tổng hợp"). Đây là chênh lệch
**đã biết trước**, không phải lỗi remap — theo `newhsk1-book-content.md` §Tổng kết, bản thân
sách chỉ bóc được ≈249 生词 (đã nêu rõ "词汇匹配新大纲300词，略有扩展" — sách tự nhận nhắm ~300
nhưng nội dung 15 课 in ra chỉ chạm ≈249, chưa kể workbook "学练手册" đi kèm có thể bổ sung
thêm từ ngoài 15 课 chính). 245 = 249 trừ 4 từ trùng lặp giữa các 课 (mục trên).

**Buổi có 生词 bị flag "OCR sót, cần đối chiếu bản gốc"** (theo Ghi chú OCR trong
`newhsk1-book-content.md`):
- **Buổi 08** (L8, trang 72 mục 8-4): hộp 生词 gốc mất một phần do ảnh quét; 能/到 phục hồi từ
  hội thoại + phần giải thích ngữ pháp — độ tin cậy cao nhưng không chụp trực tiếp từ hộp từ
  vựng. Cần đối chiếu `raw/新HSK1教程3.0.pdf` khi build buổi 08.
- **Buổi 11** (L13, trang 112 mục 13-2): hộp 生词 đầu bài không hiện trong bản quét (chỉ thấy
  phần ngữ pháp); danh sách phục hồi từ hội thoại + hộp 13-4/13-6 rõ hơn ở trang sau. Cần đối
  chiếu bản gốc khi build buổi 11.

Nếu đối chiếu PDF gốc phát hiện thêm từ bị OCR bỏ sót (ở buổi 08/11, hoặc 本/下雪 ở mục trên),
bổ sung trực tiếp vào bảng buổi tương ứng + cập nhật lại tổng số trong checklist này.
