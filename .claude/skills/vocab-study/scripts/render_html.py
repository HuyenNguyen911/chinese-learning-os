...
.xlate-pop a:hover{text-decoration:underline;}
...
/* ===== Bôi đen dịch nhanh (Google Translate zh→vi) ===== */
var _xlatePop=null;
function xlateHide(){if(_xlatePop){_xlatePop.remove();_xlatePop=null;}}
function xlateShow(text,rect){
  xlateHide();
  var p=document.createElement('div');p.className='xlate-pop';
  p.innerHTML='<span class="xl-loading">Đang dịch…</span>';
  document.body.appendChild(p);
  var top=Math.min(rect.bottom+8,window.innerHeight-60);
  var left=Math.max(8,Math.min(rect.left,window.innerWidth-p.offsetWidth-12));
  p.style.top=top+'px';p.style.left=left+'px';
  _xlatePop=p;
  var gtUrl='https://translate.google.com/?sl=zh-CN&tl=vi&text='+encodeURIComponent(text)+'&op=translate';
  var link='<a href="'+gtUrl+'" target="_blank" rel="noopener">Mở Google Translate ↗</a>';
  fetch('https://translate.googleapis.com/translate_a/single?client=gtx&sl=zh-CN&tl=vi&dt=t&q='+encodeURIComponent(text))
    .then(function(r){return r.json();})
    .then(function(data){
      var out=data[0].map(function(x){return x[0];}).join('');
      if(_xlatePop===p)p.innerHTML=esc(out)+link;
    })
    .catch(function(){
      if(_xlatePop===p)p.innerHTML='(không dịch được — cần mạng)'+link;
    });
}
document.addEventListener('mouseup',function(e){
  if(_xlatePop&&_xlatePop.contains(e.target))return;
  var sel=window.getSelection();
  var text=sel?sel.toString().trim():'';
  if(!text||text.length>200||!/[一-鿿]/.test(text)){xlateHide();return;}
  var rect=sel.getRangeAt(0).getBoundingClientRect();
  if(!rect||(rect.width===0&&rect.height===0)){xlateHide();return;}
  xlateShow(text,rect);
});
document.addEventListener('mousedown',function(e){
  if(_xlatePop&&!_xlatePop.contains(e.target))xlateHide();
});
document.addEventListener('keydown',function(e){if(e.key==='Escape')xlateHide();});
"""
