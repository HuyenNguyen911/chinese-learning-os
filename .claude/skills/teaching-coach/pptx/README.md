...
2. Ánh xạ nội dung sang các block JSON bên dưới — mỗi ý một slide, action title.
   Dùng `word_pair` cho BẤT KỲ nhóm 2 từ nào — **từ vựng chính (生词) hay mở
   rộng (生词拓展) đều áp dụng như nhau** (2026-09-12, buổi 10 HSK1: từng chỉ
   ghi rule này cho từ mở rộng, nhưng lỗi thực tế lại xảy ra ở từ chính — ghép
   máy móc 穿+女, 男+那儿 chỉ cho đủ 2 từ/slide, bị user nhắc sửa): ghép cặp
   theo **CÙNG CHỦ ĐỀ NHỎ/LIÊN QUAN NHAU** — đối lập nghĩa (深色/浅色, 女/男),
   collocation xuất hiện chung 1 câu trong bài (杯子+这边 vì "杯子在这边"), hoặc
   cùng nhóm chức năng (钱+块 cùng là đơn vị tiền) — không ghép máy móc theo
   thứ tự liệt kê có sẵn, thứ tự gốc thường xen kẽ danh từ/tính từ/đại từ
   không liên quan nhau (xem ví dụ ở bảng loại slide bên dưới).
2b. **Thứ tự khối nội dung trong 1 buổi (chốt theo Buổi 8, 2026-08-06; sửa phần
   luyện tập 2026-09-14 buổi 12 HSK1):**
   ...
   **语法**, với MỖI điểm ngữ pháp đi kèm ngay 1 slide bài tập riêng ngay sau nó
   (không dồn nhiều điểm ngữ pháp vào chung 1 slide luyện tập cuối buổi — feedback
   "sau mỗi phần ngữ pháp tôi cần 1 slide bài tập, chứ không phải dồn 3 ngữ pháp
   cho 1 slide bài tập"): 语法 điểm 1 → `exercise` điểm 1 → 语法 điểm 2 →
   `exercise` điểm 2 → ... Tức là dạy hết từ mới TRƯỚC khi học viên gặp trong
   bài, không xen kẽ "hết 1 đoạn文 → ra từ luôn" theo thứ tự sách gốc (cách đó
   từng bị coi là sai khi làm Buổi 9), VÀ mỗi điểm ngữ pháp phải luyện ngay sau
   khi học, không gộp luyện tập dồn về cuối buổi.
2c. **Từ là trọng tâm ngữ pháp có slide `语法` riêng giải thích sâu → KHÔNG cần
   thêm slide `生词`/`wordcard`/`word_pair` riêng cho từ đó nữa** (2026-09-12,
   buổi 11 HSK1) — vd 可以/一下/给 đã có slide `grammar` (`groups[]`) giảng kỹ
   kèm ví dụ, đưa thêm 1 slide từ vựng riêng cho chính 3 từ này là trùng lặp
   nội dung không cần thiết. Các từ vựng khác trong cùng câu (không phải trọng
   tâm ngữ pháp, vd 问题/打电话 đi cùng 可以) vẫn lên slide `生词` bình thường.
2d. **Ví dụ minh hoạ nên có chủ ngữ rõ ràng, tránh câu mệnh lệnh cụt lủn**
   (2026-09-12, buổi 11 HSK1, user sửa "ví dụ nên đầy đủ chủ vị ngữ, đừng có
   ngắn cộc lốc") — câu kiểu "请给我一杯茶。" (mệnh lệnh, ẩn chủ ngữ) tuy đúng
...
4. **Duyệt nội dung với user trước khi sinh audio.** Trình bày nội dung text (title +
   ...
   ⚠️ **`PermissionError` khi `build_deck.py` ghi file KHÔNG LUÔN LUÔN do PowerPoint chưa
   đóng (2026-09-19, buổi 14 HSK1):** nếu repo nằm trong thư mục đồng bộ OneDrive, chính
   OneDrive cũng có thể tạm khóa file `.pptx` vài giây-vài chục giây sau khi ghi/mở (đang
   hash/upload) — `Get-Process POWERPNT | Stop-Process` không giải quyết được trường hợp
   này vì POWERPNT không hề chạy. Xác nhận bằng cách thử mở file `r+b` trực tiếp bằng
   Python; nếu vẫn `PermissionError` dù không có tiến trình PowerPoint nào — đây là OneDrive
   khóa, không phải lỗi code. Cách xử lý: lặp retry cách nhau vài giây tới khi ghi được
   (dùng Bash `run_in_background` + vòng lặp `until ... ; do sleep 3; done`, KHÔNG dùng
   `sleep` dài trực tiếp trong lệnh chính — bị chặn), không cần debug thêm renderer.
...
⚠️ **Sàn bề rộng ảnh tăng lên + chặn crop quá tay (2026-09-19, buổi 14 HSK1 — feedback "ảnh bị cắt dọc", tái diễn từ các buổi trước):** sàn cũ `text_w = min(col_w - INNER_GAP - 1.4in, TEXT_COL_W)` để lại quá ít bề rộng cho ảnh khi cả 2 từ đều có `example` — ảnh bị ép còn ~1.9in trong khi `area_h` ~5.3in (tỉ lệ ~1:2.8), khiến `fit="cover"` phải cắt >35% mỗi bên, cắt cụt mất đầu/chủ thể ảnh chân dung. Đã sửa 2 lớp: (1) `_place_image` giờ có `CROP_FRAC_LIMIT = 0.20` — nếu "cover" cần cắt hơn 20% một chiều, tự rơi về "contain" (letterbox, không cắt) thay vì cắt cụt; (2) bề rộng cột chữ (`text_w`) không còn dùng sàn cố định mà **đo THẬT** bề rộng 1 dòng cần cho hz/pos/py/vn (dùng `_text_width_pt`, KHÔNG tính câu `example` vào phép đo — câu ví dụ được phép tự xuống dòng, đã cộng đúng số dòng wrap vào chiều cao) — câu càng ngắn thì text_w càng hẹp, ảnh càng được nhường nhiều chỗ hơn. Lưu ý khi đo: nếu từ có `pos`, phải đo phần `pos` ở ĐÚNG cỡ chữ thật vẽ (15pt, không bold) tách riêng khỏi hz (50pt bold) — đo gộp cả cụm ở cỡ 50pt từng làm `needed_pt` bị thổi phồng gần gấp đôi, vô tình đẩy `text_w` lên kịch trần `TEXT_COL_W` (hiệu ứng ngược hẳn với mục tiêu "ảnh to hơn").
⚠️ **Câu ví dụ (`example`) ở nhánh ảnh `"left"`/`"right"` giờ nằm DẢI RIÊNG FULL-WIDTH bên dưới cả ảnh+từ, không còn bó trong cột chữ hẹp cạnh ảnh (2026-09-19, buổi 14 HSK1 — feedback "đặt ví dụ dưới ảnh, đỡ bị ép rớt dòng"):** đo wrap theo `col_w` (đầy đủ bề rộng cột) thay vì `text_w` hẹp, nên câu ví dụ dài vẫn gọn trong 1-2 dòng thay vì rớt xuống 3-4 dòng. Ảnh dùng chiều cao `row1_h = area_h - chiều_cao_dải_ví_dụ - gap` (không còn full `area_h`) — nghịch lý là ảnh vẫn TO HƠN trước vì bề rộng ảnh được nhường nhiều hơn hẳn (không phải chia sẻ chỗ với ví dụ nữa). Nhánh `top_stack`/không-ảnh giữ nguyên hành vi cũ (ví dụ nối tiếp ngay dưới info, trong cùng cột hẹp).
⚠️ **Mặc định `image_pos` cho từ THỨ 2 trong cặp đổi thành `"right"` (2026-09-19, buổi 14 HSK1 — feedback "các từ không đối xứng à?"):** trước đó cả 2 từ đều mặc định `"left"` (ảnh nằm bên trái CỦA CHÍNH CỘT ĐÓ) — với từ 2 (cột bên phải dải phân cách), "trái của cột" lại chính là cạnh SÁT dải phân cách (cạnh trong), trong khi ảnh từ 1 nằm rìa ngoài-trái slide — 2 ảnh lệch nhau (1 rìa ngoài, 1 sát giữa) dù mỗi cột riêng lẻ đều đúng rule. Giờ khi `n==2`, từ index 1 mặc định `image_pos: "right"` (ảnh ra rìa ngoài-phải, chữ vào giữa) — đối xứng gương với từ 1. Chỉ áp dụng khi JSON không tự khai `image_pos` (khai tay luôn được tôn trọng). |
...
⚠️ **`image_layout: "top_left"` — layout THAY THẾ opt-in, KHÔNG phải mặc định
(2026-09-19, buổi 14 HSK1, theo mockup user gửi trong chat):** khi khai field
này trên 1 slide `vocab` chế độ thẻ (có `image`), ảnh thu nhỏ về góc trên-trái
(~5.2in ngang, cao tối đa ~3.6in), 2-3 thẻ từ xếp cạnh ảnh cùng hàng trên, câu
ví dụ kéo full-width ở dải dưới cùng — khác hẳn layout 3-cột mặc định ở trên
(ảnh cao hết `area_h` bên trái). Chỉ dùng khi user chủ động xin kiểu bố cục
này; KHÔNG tự ý áp dụng cho slide khác hay buổi khác khi chưa được yêu cầu —
lần đầu thêm field này user chỉ chốt dùng cho riêng buổi 14, không đổi mặc
định chung của renderer.
...
**KHÔNG lặp lại y nguyên** 汉字+nghĩa đã hiện to sẵn trong thân slide (vd tiêu đề
mô tả bối cảnh/ngữ pháp liên quan, hoặc gộp nhãn "生词 · Trong lớp học" khi đây là
slide 生词 đầu tiên của 1 nhóm chủ đề mới). Nếu title chỉ là "hz1 · hz2 — nghĩa1 /
nghĩa2" giống hệt nội dung bên dưới → **bỏ hẳn `title`** (không khai field, hoặc
`title: ""`) — header chỉ còn dải màu + tab `kicker`, đây là lựa chọn CÓ CHỦ ĐÍCH
tránh lặp nội dung, không phải "quên khai". Renderer đã bỏ fallback mặc định
"Từ vựng mới"/"Hồ sơ" (code cũ tự điền khi thiếu `title`) — thiếu `title` giờ chỉ
hiện header trống + kicker, không còn chữ lạ.
...
- Hội thoại (`dialogue`): 3 giọng theo người nói (`DIALOGUE_VOICES`), mặc định
  ...
  **⚠️ Nhãn giống nhau (vd "服务员") lặp lại ở NHIỀU 课文 KHÁC NHAU không có
  nghĩa cùng 1 người/giới tính** (2026-09-12, buổi 11 HSK1): sách hay dùng lại
  nhãn vai chung chung (服务员/女士/先生...) cho các nhân vật khác nhau ở từng
  课文 — vd 课文2 "服务员" là nữ (phục vụ quán cà phê), 课文3 "服务员" lại là nam
  (phục vụ nhà hàng). Phải tự kiểm tra ảnh minh hoạ trong sách RIÊNG từng 课文
  để xác nhận giới tính, không suy diễn theo nhãn hay tin vào kết quả
  auto-assign của lần 课文 trước.
...
>
> ⚠️ **Đổi `voices`/nội dung của 1 slide dialogue ĐÃ có audio (không đổi thứ tự
> slide) vẫn cần tự xoá đúng file mp3 slide đó** (2026-09-12, buổi 11 HSK1) —
> script chỉ kiểm tra file đã TỒN TẠI, không so sánh nội dung/voices đã đổi hay
> chưa, nên sửa `voices` xong chạy lại `slide_audio.py` mà không xoá file cũ sẽ
> bị bỏ qua, giữ nguyên audio giọng CŨ dù JSON đã đổi. Chỉ cần `rm` đúng
> `assets/audio/slideNN.mp3` của slide đó (không cần xoá cả thư mục) rồi chạy
> lại — khác với case đổi THỨ TỰ slide ở trên (case đó phải xoá hết).
>
...
PHẢI kiểm thêm 3 điểm sau — trong buổi 6 phải fetch lại 6/27 ảnh vì bỏ sót:
...
3. **Không hiện tên thương hiệu/cửa hàng thật rõ ràng, không nội dung chính trị/tôn
   giáo/quảng cáo rượu bia không liên quan** (2026-09-12, buổi 10 HSK1 — phải fetch lại
   3-4 lần cho query "cửa hàng/shop" chung chung mới sạch). Đã gặp: ảnh cửa hàng có biển
   hiệu thật rõ tên riêng ("HUGO ACCESSORIES", "BELBIN'S GROCERY" + số điện thoại), áo
   thun in hình mang tính chính trị, ảnh có cờ/bối cảnh đài tưởng niệm không liên quan gì
   đến từ đang dạy (chỉ vì query chung chung "mặc/wear"), pano quảng cáo bia rượu chiếm
   nửa khung hình. Với query đại trà kiểu "shop"/"store"/"cửa hàng" — luôn kiểm ảnh có
   chữ/biển hiệu lớn hiện rõ tên riêng hay không trước khi chốt; nếu có, thử query cụ thể
   hơn (vd tên địa danh du lịch nổi tiếng — Hội An, chợ Bến Thành) hoặc đổi hẳn góc chụp
   (đường phố tổng thể thay vì cận cảnh 1 cửa hàng có tên).
...
không thấy key thì xin user 1 lần rồi lưu lại vào đó):
```bash
